...
def create_email_queue(channel):
    ...
    channel.queue_bind(exchange=exchangename, queue=e_queue_name, routing_key='warranty.update')


# function to create service update queue
def create_su_queue(channel):
    print('amqp_setup:create_su_queue')
    su_queue_name = 'ServiceUpdate'
    channel.queue_declare(queue=su_queue_name, durable=True) # 'durable' makes the queue survive broker restarts
    channel.queue_bind(exchange=exchangename, queue=su_queue_name, routing_key='service.update')

# function to create service update queue
def create_shipping_queue(channel):
    print('amqp_setup:create_shipping_queue')
    shipping_queue_name = 'Shipping'
    channel.queue_declare(queue=shipping_queue_name, durable=True) # 'durable' makes the queue survive broker restarts
    channel.queue_bind(exchange=exchangename, queue=shipping_queue_name, routing_key='shipping.update')
