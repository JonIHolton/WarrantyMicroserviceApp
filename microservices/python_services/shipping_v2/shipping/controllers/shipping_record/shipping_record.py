...
@api_shipping_record.route('/shippingrecords/<case_id>/<order_code>', methods=['PUT'])
def update_shipping_record(case_id,order_code):
    data = request.json
    result, status = ShippingRecordService.update_shipping_record(case_id, data, order_code)
    return jsonify(result), status
